...
import { GET_WISHLISTS } from "../../../lib/api/wishlists";
import { Wishlist, WishlistResult } from "../../../lib/api/wishlists";
import { useActiveCustomer } from "../../../lib/customer-context";
import {
  ExportDialog,
  ExportOptions,
} from "../../../components/ui/export-dialog";
import { Download, Plus, Search, Pencil, Heart } from "lucide-react";
...
function WishlistsPageContent() {
  ...
  useEffect(() => {
    ...
    router.push(`/dashboard/wishlists?${params.toString()}`);
  ...
  const wishlistsResult: WishlistResult = data?.getWishlists || {
    wishlists: [],
    pagination: {
      totalItems: 0,
      totalItemOnPage: 0,
  ...
  const wishlists: Wishlist[] = wishlistsResult.wishlists;
  const pagination = wishlistsResult.pagination;
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
            Showing wishlists for: {activeCustomer.firstName}{" "}
        ...
        <div className="flex gap-2">
          ...
          <button
            onClick={() => router.push("/dashboard/wishlists/new")}
            ...
            {wishlists.length > 0 ? (
              wishlists.map((wishlist: Wishlist) => (
                ...
                >
                  ...
                  <td className="hidden md:table-cell px-6 py-4 text-sm text-gray-900 dark:text-white max-w-xs truncate">
                    {wishlist.notes || "-"}
                  </td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm">
                    <div className="flex items-center gap-2">
                      <button
                        onClick={() =>
                          router.push(
                            `/dashboard/wishlists/${wishlist.id}/edit`,
                          )
                        }
              ...
              <tr>
                ...
                >
                  No wishlists found for the selected customer.
                </td>
              </tr>
            )}
          </tbody>
