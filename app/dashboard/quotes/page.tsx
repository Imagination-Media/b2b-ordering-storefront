...
import { GET_QUOTES } from "../../../lib/api/quotes";
import { Quote, GetQuotesResult } from "../../../lib/api/quotes";
import { useActiveCustomer } from "../../../lib/customer-context";
import {
  ExportDialog,
  ExportOptions,
} from "../../../components/ui/export-dialog";
import { Download, Plus, Search, Pencil, FileText } from "lucide-react";
...
function QuotesPageContent() {
  ...
  const [filterField, setFilterField] = useState<string>(
    searchParams.get("filterField") ?? "identifier",
  );
  ...
  const urlFilterField = searchParams.get("filterField") ?? "identifier";
  ...
  if (error)
    return (
      <div className="text-red-500">Error loading quotes: {error.message}</div>
    );

  const quotesResult: GetQuotesResult = data?.getQuotes || {
    quotes: [],
    pagination: {
      totalItems: 0,
      totalItemOnPage: 0,
  ...
  const quotes: Quote[] = quotesResult.quotes;
  const pagination = quotesResult.pagination;
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
            Showing quotes for: {activeCustomer.firstName}{" "}
          ...
          <tbody className="bg-white dark:bg-gray-900 divide-y divide-gray-200 dark:divide-gray-700">
            {quotes.length > 0 ? (
              quotes.map((quote: Quote) => (
                ...
                >
                  ...
                  <td className="hidden md:table-cell px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">
                    {quote.location
                      ? `${quote.location.name} (${quote.location.code})`
                      : "-"}
                  </td>
                  <td className="hidden md:table-cell px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">
              ...
              <tr>
                ...
                >
                  No quotes found for the selected customer.
                </td>
              </tr>
            )}
          </tbody>
